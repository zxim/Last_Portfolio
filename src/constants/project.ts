...
export const projectData: IProjectProps[] = [
  // 0. n8n 기반 자동화 구축
  {
    ...
    intro: '사내 DRM으로 보호되는 파일을 외부 협업(구글 드라이브)에 안전하게 활용하기 위해, 인가된 게이트웨이 서버에서만 DRM을 해제해 업로드하도록 통제하는 보안 자동화 플랫폼입니다. 임직원은 메일 OTP 인증 후 별도 서버에 파일을 업로드하기만 하면 Python 백엔드가 Google Drive API로 직접 적재하며, 모든 파일의 소유권을 회사가 보유한 채 다운로드·인쇄·복사를 원천 차단합니다. 이를 통해 1인당 연 70만원(팀 단위 연 1천만원 이상)에 달하던 상용 협업 솔루션 라이선스 비용을 전액 무료화했습니다.',
    func: [
