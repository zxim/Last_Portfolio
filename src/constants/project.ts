...
export const projectData: IProjectProps[] = [
  // 0. n8n 기반 자동화 구축
  {
    ...
    issues: [
      ...
      },
      {
        ...
        solving: '자체 파일 업로드 게이트웨이 서버를 구축하여 MS 파일을 표준화된 방식으로 수신 후, Python 백엔드가 Google Drive API로 안정적으로 업로드하도록 구현'
      },
      {
      ...
      {
        issue: '대량 파일 업로드 시 Google Drive API 호출 한도(Rate Limit) 초과 및 OAuth 토큰 만료로 작업이 중단되는 문제 발생',
        solving: '지수 백오프(Exponential Backoff) 기반 재시도와 토큰 자동 갱신 로직을 Python 백엔드에 구현하여 대량 처리 안정성을 확보'
      }
    ],
    reason: "구글 기본 관리 기능의 한계를 넘기 위해 Google Drive API를 직접 제어하는 방식을 택했고, 프론트엔드는 React, 파일 제어 로직과 자동화는 Python 백엔드로 구현했습니다. 여기에 MySQL을 정책 저장소로 연동해 실시간 파일 상태 기록과 화이트리스트 대조를 자동화하고, 메일 OTP 인증으로 인가된 사용자만 게이트웨이에 접근하도록 통제함으로써 상용 솔루션 없이도 저비용·고효율의 맞춤형 보안 엔진을 구현했습니다.",
    learned: '보안은 단순히 통제하는 것이 아니라, 사용자가 인지하지 못하는 사이 시스템이 자동으로 보호하는 것이 핵심임을 배웠습니다',
    intro: '사내 DRM으로 보호되는 파일을 외부 협업(구글 드라이브)에 안전하게 활용하기 위해, 인가된 게이트웨이 서버에서만 DRM을 해제해 업로드하도록 통제하는 보안 자동화 플랫폼입니다. 임직원은 메일 OTP 인증 후 별도 서버에 파일을 업로드하기만 하면 Python 백엔드가 Google Drive API로 직접 적재하며, 모든 파일의 소유권을 회사가 보유한 채 다운로드·인쇄·복사를 원천 차단합니다. 현재 약 60명의 임직원이 사용 중이며, 1인당 연 70만원에 달하던 상용 협업 솔루션 라이선스 비용을 전액 무료화해 연 4천만원 이상의 예산을 절감했습니다.',
    func: [
      'React 프론트엔드 · Python 백엔드 · MySQL 기반의 파일 업로드 게이트웨이 구축',
      'Google Drive API 연동을 통한 파일 적재·소유권·권한 중앙 제어 자동화',
      'DRM 적용 파일을 인가된 게이트웨이 서버에서만 복호화·업로드하도록 통제',
      '메일 OTP 인증 기반 사용자 식별 및 외부 유출 경로(복사/인쇄/다운로드) 원천 차단',
      'MySQL 연동을 통한 동적 화이트리스트 검증 및 비인가 협업자 자동 숙청 로직 구현',
      '자체 게이트웨이 서버 운영으로 상용 협업 솔루션 라이선스 비용 전액 절감(약 60명 사용, 연 4천만원 이상)'
    ],
  },
  ...
  // 보안장비 통합 점검·정책 자동화 (회사 프로젝트)
  {
    ...
    team: '회사 프로젝트',
    contribution: { dev: '100%', infra: '100%', security: '100%' },
    stacks: ['React', 'Python', 'MySQL', 'Docker', 'n8n'],
    issues: [
      {
      ...
      },
      {
        ...
        solving: 'Docker로 점검 작업을 컨테이너화해 실행 환경을 격리하고, n8n 워크플로우로 일일 로그 요약과 월간 정책 점검을 주기적으로 자동 실행했습니다.'
      }
    ],
    ...
    func: [
      ...
      'Docker 컨테이너화로 점검 환경 격리, n8n 워크플로우로 일/월 단위 점검 자동 스케줄링',
